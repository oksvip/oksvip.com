	...
	static function optionsframework_fields() {
		...
		foreach ( $options as $value ) {
			...
			if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) ) {
				...
				if ( isset( $value['name'] ) ) {
					$output .= '<h4 class="heading tm-article-subtitle">' . esc_html( $value['name'] ) . '</h4>' . "\n";
				}
		...
		case "colorradio":
			...
			foreach ( $value['options'] as $key=>$key ) {
				...
				if ( $val != '' ) {
					if ( $val == $key ) {
						$selected = ' optionsframework-radio-img-selected';
						$checked = ' checked="checked"';
					}
				}
				$output .= '<input type="radio" id="' . esc_attr( $value['id'] .'_'. $key) . '" class="optionsframework-radio-img-radio" value="' . esc_attr( $key ) . '" name="' . esc_attr( $name ) . '" '. $checked .' />';
				$output .= '<div class="optionsframework-radio-img-label">' . esc_html( $key ) . '</div>';
				$output .= '<a style="background-color:#'.$key.';" href="javascript:;" class="optionsframework-radio-img-img optionsframework-radio-color' . $selected .'" onclick="document.getElementById(\''. esc_attr($value['id'] .'_'. $key) .'\').checked=true;"></a>';
			}
			break;
		...
		case "radio":
			...
			case "checkbox":
				...
				$output .= '<input id="' . esc_attr( $value['id'] ) . '" class="sxoptions-checkbox of-input" type="checkbox" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" '. checked( $val, 1, false) .' />';
				$output .= '<div class="checkbox-wrapper"><label class="sxexplain" for="' . esc_attr( $value['id'] ) . '"></label></div>';
				$output .= '<div class="sxexplain-checkbox">' . wp_kses( $explain_value, $allowedtags ) . '</div>'."\n";
				break;
