...
function SuStatic() {
    ...
    wp_register_style( 'fontello', get_template_directory_uri() . '/includes/font-awesome/css/fontello.css', array('Grace-style'), THEME_VERSION, 'all' );
    wp_register_style( 'animation', get_template_directory_uri() . '/includes/font-awesome/css/animation.css', array('Grace-style'), THEME_VERSION, 'all' );
    wp_register_style( 'fontello-ie7', get_template_directory_uri() . '/includes/font-awesome/css/fontello-ie7.css', array('Grace-style'), THEME_VERSION, 'all' );
    wp_register_style( 'carousel', get_template_directory_uri() . '/includes/css/owl.carousel.css', array('Grace-style'), THEME_VERSION, 'all' );
    wp_register_style( 'carousel-theme', get_template_directory_uri() . '/includes/css/owl.theme.css', array('Grace-style'), THEME_VERSION, 'all' );
    wp_register_style( 'animate', get_template_directory_uri() . '/includes/css/animate.css', array('Grace-style'), THEME_VERSION, 'all' );
    ...
    if ( !is_admin() ) {
        ...
        wp_script_add_data( 'fontello-ie7', 'conditional', 'lte IE 7' );
...
function suxing_get_share(){
    ...
    return $html.'<a class="share-links bds_more icon-plus-1" data-cmd="more"></a>';
}
...
function mee_insert_posts( $atts, $content = null ){
    $wow = isset( $GLOBALS['wow_single_embed_post'] ) ? $GLOBALS['wow_single_embed_post'] : '';
    ...
    foreach ($inset_posts as $key => $post) {
        setup_postdata( $post );
        $content .= '<div class="warp-post-embed '.$wow.'"><a href="' . get_permalink() . '" target="_blank" class="post-embed-bg" style="background-image:url('.post_thumbnail_src().')"></a><a href="' . get_permalink() . '" target="_blank"><span class="embed-title">'.get_the_title().'</span><em class="embed-content">'. wp_trim_words(get_the_excerpt(), 60 ) .'</em><div class="embed-mata"><i class="icon-eye-3"></i>'.get_post_meta($post->ID, 'views', true).' <i class=" icon-heart"></i>'.get_post_meta($post->ID,'suxing_ding',true).'</div></a></div>';
    }
    wp_reset_postdata();
    return $content;
}
