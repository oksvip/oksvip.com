...
function alu_smilies_src($img_src, $img, $siteurl) {
    $img = rtrim($img, "gif");
    return get_template_directory_uri() . '/includes/wp-alu/static/img/' . $img . 'gif';
}
...
function alu_get_wpsmiliestrans() {
    ...
    foreach ($wpsmilies as $alt => $src_path) {
        ...
        $output .= '<a class="add-smily" data-action="addSmily" data-smilies="' . $alt . '"><img class="wp-smiley" src="' . get_template_directory_uri() . '/includes/wp-alu/static/img/' . $src_path .'" /></a>';
    }
    return $output;
}
